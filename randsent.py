...
class Grammar:
    ...
    def _load_rules_from_file(self, grammar_file):
        ...
        with open(grammar_file, 'r') as f:
            for line in f:
                ...
                probs, lhs, rhs = line.split('\t')
                rhs = rhs.split()
                
                if lhs not in self.rules:
                    self.rules[lhs] = [(rhs, float(probs))]
                else:
                    self.rules[lhs].append((rhs, float(probs)))
    ...
    def sample(self, derivation_tree, max_expansions, start_symbol):
        ...
        expand_counter, depth_counter = 0, 0
        sentence, return_tree = [], []
        queue = [(start_symbol, depth_counter)]
        while queue:
            tag, current_depth = queue.pop(0)
        
            if tag in self.rules:
                expand_counter += 1
                depth_counter += 1
                return_tree.append('(' + tag + ' ')
                if expand_counter == max_expansions:
                    queue = [('...', depth_counter)] + queue
                    break
                weights, rhs = [], []
                ...
                next_rule = random.choices(rhs, weights)[0]
                queue = [(x, depth_counter) for x in next_rule] + queue
            
            else:
                if queue:
                    _ , depth_counter = queue[0]
                    i = current_depth - depth_counter
                    sentence.append(tag + ' ')
                    return_tree.append(tag + ')' * i + '\n')
                else:
                    sentence.append(tag)
                    return_tree.append(tag + ')')
        if queue:
            depth_counter = 1
            tag, pre_depth = queue.pop(0)
            while queue:
                next_tag, next_depth = queue.pop(0)
                sentence.append('...' + ' ')
                i = pre_depth - next_depth
                return_tree.append('...' + ')' * i + '\n')
                tag, pre_depth = next_tag, next_depth
            sentence.append(tag)
            return_tree.append(tag + ')' +'\n')
            
        if depth_counter > 1:
            return_tree.append(')' * (depth_counter - 1))
            
        if derivation_tree:
            return ''.join(return_tree)
        else:
